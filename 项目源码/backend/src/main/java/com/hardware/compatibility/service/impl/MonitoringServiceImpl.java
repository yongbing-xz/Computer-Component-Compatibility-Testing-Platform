    ...
    public Map<String, Object> getErrorStatistics(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        synchronized (errorHistory) {
            List<Map<String, Object>> filteredErrors = errorHistory.stream()
                .filter(error -> {
                    Object ts = error.get("timestamp");
                    if (!(ts instanceof LocalDateTime)) return false;
                    LocalDateTime errorTime = (LocalDateTime) ts;
    ...
    public Map<String, Object> checkSystemResources() {
        ...
        try {
            ...
            try {
                ...
                // 非HotSpot VM或权限不足时使用回退策略
                OperatingSystemMXBean osBean = ManagementFactory.getOperatingSystemMXBean();
                double loadAvg = osBean.getSystemLoadAverage();
                cpuUsage = loadAvg >= 0 ? loadAvg : 0;
            }
            ...
            // 系统负载（Windows不支持，可能返回-1）
            try {
                double loadAverage = ManagementFactory.getOperatingSystemMXBean().getSystemLoadAverage();
                resources.put("systemLoadAverage", loadAverage >= 0 ? loadAverage : "N/A (Windows)");
            } catch (Exception e) {
                resources.put("systemLoadAverage", "N/A");
            }
